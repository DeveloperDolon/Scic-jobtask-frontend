        ...
        <div className="h-full bg-gradient-to-tl from-green-400 to-indigo-900 w-full py-16 px-4">
            <div className="flex flex-col items-center justify-center">
                ...
                <Link to={"/"} className="flex flex-col items-center gap-2">
                    ...
                    <h1 className="md:text-2xl text-xl font-bold text-white text-center">TODO TAKER</h1>
                </Link>
                ...
                <div className="bg-white shadow rounded lg:w-1/3  md:w-1/2 w-full p-10 mt-16">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="mt-6">
                            ...
                            <input {...register("photo")} aria-label="enter email adress" role="input" type="text" className="bg-gray-200 border rounded focus:outline-none text-xs font-medium leading-none text-gray-800 py-3 w-full pl-3 mt-2" />
                        </div>
                        <div className="mt-6">
                            ...
                            {errors.email && <span className="text-red-500 md:text-sm text-xs pt-1">This field is required</span>}
                        </div>
                        <div className="mt-6  w-full">
                            <lable className="text-sm font-medium leading-none text-gray-800">Password</lable>
                            <div className="relative flex items-center justify-center">
                                <input {...register("password", { required: true, pattern: /^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]/, maxLength: 20, minLength: 8 })} aria-label="enter Password" role="input" type={`${showPass ? "text" : "password"}`} className="bg-gray-200 border rounded focus:outline-none text-xs font-medium leading-none text-gray-800 py-3 w-full pl-3 mt-2" />
                                <div className="absolute right-0 mt-2 mr-3 cursor-pointer">
                                    <svg onClick={handleShowPassword} width={16} height={16} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path
                            ...
                            {errors.password?.type === "required" && <span className="text-red-500 md:text-sm text-xs pt-1">This field is required</span>}
                            {errors.password?.type === "pattern" && <span className="text-red-500 md:text-sm text-xs pt-1">Password must include one digit, one special character, one uppercase and one lowercase letter.</span>}
                            {errors.password?.type === "maxLength" && <span className="text-red-500 md:text-sm text-xs pt-1">Password can&apos;t be more then 20 character.</span>}
                            {errors.password?.type === "minLength" && <span className="text-red-500 md:text-sm text-xs pt-1">Password can&apos;t be less then 8 character.</span>}
                        </div>
                        <div className="mt-8">
